...
// (SPARSEGPTQ 1:4 F2B OG) Mayber threads=32 or 64, reg pressure is pretty high
__global__ void sparsegptq14_f2b_intrablock_kernel(
...
std::tuple<torch::Tensor, torch::Tensor> sparsegptq14_solver_cuda(
    ...
    for (int64_t block_start = 0; block_start < C; block_start += block_size) {
        ...
        sparsegptq14_f2b_intrablock_kernel<<<grid, threads, 0, stream>>>(
            W.data_ptr<float>(),
            U.data_ptr<float>(),
            Uinv.data_ptr<float>(),
            M.data_ptr<uint32_t>(),

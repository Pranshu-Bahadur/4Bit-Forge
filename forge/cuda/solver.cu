...
// ---------------------------------------------------------------------------
// QMeta Decoding (log2 Q8.8 -> scale, inv_scale, qzero, maxq)
// ---------------------------------------------------------------------------
...
__device__ __forceinline__ void decode_qmeta(
    ...
    // Bits-per-group (global for now; flags reserved for per-group bits)
...
// Scalar quantization for a single element
__device__ __forceinline__ void quantize_scalar(
    ...
    deq = __fmaf_rn(static_cast<float>(q), s, -q0 * s);  // (q - q0) * s
    e   = deq - x;
    q_out = static_cast<uint8_t>(q);
}

// Vectorized (float4) quantization over 4 elements
__device__ __forceinline__ void quantize_vec4(
    ...
    // Little-endian packing: x = LSB, w = MSB
...
// ---------------------------------------------------------------------------
// Main GPTQ Kernel (block-local part of Algorithm 1)
// ---------------------------------------------------------------------------
//
// Templated on:
//   - WARPS_PER_BLOCK: number of warps (threads = WARPS_PER_BLOCK * 32)
//   - VECTORIZED: use float4/uint32 path vs scalar fallback
//
...
__global__ void gptq_solver_kernel(
    ...
    const int STRIDE = ((int)G + 7) & ~7; // PackBoost-style padding
    ...
    // -----------------------------------------------------------------------
    // 1. Cooperative load + decode of qmeta into shared memory
    // -----------------------------------------------------------------------
    const int total_qmeta = B * static_cast<int>(G);
    for (int idx = tid; idx < total_qmeta; idx += blockDim.x) {
        int row_in_block = idx / static_cast<int>(G); // 0..B-1
        int g            = idx % static_cast<int>(G); // 0..G-1
    ...
    // -----------------------------------------------------------------------
    // 2. Zero error_block for this block (B rows × R cols)
    // -----------------------------------------------------------------------
    if (VECTORIZED) {
        int total_vec = (B * static_cast<int>(R)) / 4;
        float4* err_vec = reinterpret_cast<float4*>(error_block);
        for (int idx = tid; idx < total_vec; idx += blockDim.x) {
            err_vec[idx] = make_float4(0.f, 0.f, 0.f, 0.f);
        }
    } else {
        int total_err = B * static_cast<int>(R);
    ...
    // -----------------------------------------------------------------------
    // 3. Serial loop over j in this block (Algorithm 1 inner loop)
    // -----------------------------------------------------------------------
    for (int j = static_cast<int>(block_start); j < static_cast<int>(block_end); ++j) {
        int row_in_block = j - static_cast<int>(block_start);

        // --- 3a. Quantize column j (entire row) and accumulate error e_j ----
        if (VECTORIZED) {
            ...
            for (int col_v = tid; col_v < R_vec; col_v += blockDim.x) {
                int g = col_v / group_size_vec; // group index

                int off = row_in_block * STRIDE + g;
                float inv_s = sm_inv_scale[off];
                float s     = sm_scale[off];
                float q0    = sm_qzero_f[off];
                int   maxq  = static_cast<int>(sm_maxq_g[off]);

                float4 x_vec = W_row_vec[col_v];

                float4 e_vec, deq_vec;
                uint32_t q_packed;
                quantize_vec4(x_vec, inv_s, s, q0, maxq, e_vec, q_packed, deq_vec);
            ...
            for (int col = tid; col < static_cast<int>(R); col += blockDim.x) {
                ...
                float   e, deq;
                uint8_t q_byte;
                quantize_scalar(x, inv_s, s, q0, maxq, e, q_byte, deq);
        ...
        // --- 3b. In-block update: W[k] += Hinv[j,k] * e_j -------------------
        ...
        if (block_rows_after_j > 0) {
            for (int rel_k = warp_id; rel_k < block_rows_after_j; rel_k += WARPS_PER_BLOCK) {
                ...
                // One lane loads alpha = Hinv[j, k], then broadcast within warp
                float alpha = 0.f;
                if (lane_id == 0) {
                    alpha = Hinv[j * static_cast<int>(C) + k];
                }
...
// ---------------------------------------------------------------------------
//
// weight:      (C, R) float32 (solver buffer, modified in-place)
// hessian_inv: (C, C) float32 (pre-inverted Hessian)
// qmeta_bytes: (C, G, 4) or (C*G, 4) uint8
//
// group_size:  typically 128
// bits:        4 (for W4)
// block_size:  <= 0 -> inferred from shared-memory limits
//
torch::Tensor gptq_solver_cuda(
    ...
    TORCH_CHECK(weight.dim() == 2, "weight must be 2D (C, R)");
    TORCH_CHECK(hessian_inv.dim() == 2, "hessian_inv must be 2D (C, C)");

    const int64_t C = weight.size(0);
    const int64_t R = weight.size(1);

    TORCH_CHECK(hessian_inv.size(0) == C && hessian_inv.size(1) == C,
                "Hinv shape mismatch: expected (C, C)");

    TORCH_CHECK(weight.scalar_type() == at::kFloat,
                "Solver requires float32 weight");
    TORCH_CHECK(hessian_inv.scalar_type() == at::kFloat,
                "Solver requires float32 Hinv");
    TORCH_CHECK(qmeta_bytes.scalar_type() == at::kByte,
                "qmeta_bytes must be uint8");

    // Infer G and ensure last dim == 4
    int64_t G = 0;
    if (qmeta_bytes.dim() == 3) {
        TORCH_CHECK(qmeta_bytes.size(0) == C,
                    "qmeta_bytes[0] must be C when dim==3");
        TORCH_CHECK(qmeta_bytes.size(2) == 4,
                    "qmeta_bytes[..., 4] expected as last dim");
        G = qmeta_bytes.size(1);
    } else if (qmeta_bytes.dim() == 2) {
        TORCH_CHECK(qmeta_bytes.size(1) == 4,
                    "qmeta_bytes.shape[1] must be 4 when dim==2");
        TORCH_CHECK(qmeta_bytes.size(0) % C == 0,
                    "qmeta_bytes.shape[0] must be a multiple of C");
        G = qmeta_bytes.size(0) / C;
    } else {
        TORCH_CHECK(false,
                    "qmeta_bytes must be 2D (C*G, 4) or 3D (C, G, 4)");
    }

    TORCH_CHECK(group_size > 0, "group_size must be > 0");

    // Flatten to (C*G, 4) for reinterpret_cast to QMetaPacked
    auto qmeta_flat = qmeta_bytes.view({C * G, 4});

    // -----------------------------------------------------------------------
    // 2. Choose block_size based on shared memory budget (A100/H100 target)
    // -----------------------------------------------------------------------
    // Each row in shared memory stores:
    //   inv_scale, scale, qzero, maxq   -> 4 * STRIDE floats
    // Total per-block dynamic shared mem: B * 4 * STRIDE * sizeof(float)
    //
    // We assume 96 KB dynamic shared mem budget per block (safe on A100/H100).
    //
    const int STRIDE = static_cast<int>((G + 7) & ~7);
    constexpr size_t SMEM_LIMIT = 96ull * 1024ull; // adjust if you opt-in more
    const size_t per_row_smem =
        4ull * static_cast<size_t>(STRIDE) * sizeof(float);

    TORCH_CHECK(
        per_row_smem <= SMEM_LIMIT,
        "gptq_solver_cuda: cannot fit even one row of qmeta in shared memory. "
        "Per-row smem = ", per_row_smem, " bytes, limit = ", SMEM_LIMIT,
        ". Reduce G or group_size."
    );

    int64_t max_block_size = static_cast<int64_t>(SMEM_LIMIT / per_row_smem);
    TORCH_CHECK(max_block_size > 0, "Internal error: max_block_size <= 0");
    ...
    // -----------------------------------------------------------------------
    // 3. Allocate outputs / temp buffers
    // -----------------------------------------------------------------------
    ...
    // Vectorized path requires R and group_size divisible by 4
    bool use_vectorized = (R % 4 == 0) && (group_size % 4 == 0);

    // -----------------------------------------------------------------------
    // 5. Block loop over j (Algorithm 1 outer loop over i=0,B,2B,...)
    // -----------------------------------------------------------------------
    for (int64_t block_start = 0; block_start < C; block_start += block_size) {
        ...
        size_t smem_bytes = per_row_smem * static_cast<size_t>(B);
        ...
        if (use_vectorized) {
            gptq_solver_kernel<WARPS_PER_BLOCK, true>
                <<<1, threads, smem_bytes, stream>>>(
                    ...
                    reinterpret_cast<const QMetaPacked*>(
                        qmeta_flat.data_ptr<uint8_t>()),
        ...
        // -------------------------------------------------------------------
        // 6. Tail update: W_tail += H_block^T @ error_block   (Algorithm 1)
        // -------------------------------------------------------------------
        if (block_end < C) {
            auto W_tail  = weight.narrow(0, block_end, C - block_end);
            auto H_block = hessian_inv
                               .narrow(0, block_start, B)
                               .narrow(1, block_end, C - block_end);
